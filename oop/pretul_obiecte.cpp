...
public:
    ...
    Object(std::string mvaluable ,float mprice){
        valuable=mvaluable;
        price=mprice;
    }
    std::string getValuable(){
        return valuable;
    }
    float getPrice(){
        return price;
    ...
    class House {
        ...
        std::string city;
        uint32_t number;
        std::vector <Object> valuables;
    public:
    House ();
        House (std::string mstreet,std::string mcity,uint32_t mnumber){
            street=mstreet;
            city=mcity;
            number=mnumber;
        }
        ...
        Object getValuable(uint32_t n){
            return valuables[n];
        }
        void add(Object a){
            valuables.push_back(a);
        }
...
bool add(std::vector<House> &a,House s){
std::string ss;
    for (int i =0;i<a.size();i++){
        ss=a[i].getStreet();
        if(ss==s.getStreet()) {
            for (int j=0;j<s.getValuablesSize();j++){
                a[i].add(s.getValuable(j));
                }
            return 0;
        }
...
int main() {
    //std::map <House,float> ff;
    // std::vector <House> aa;
    std::vector<House> all;
    std::string street, city, v;
    uint32_t number;
    float p;
    int n;
...
//        float sum = getTotalPrice(f);
//        std::cout << f.getStreet() << " ;total =" << sum << std::endl;
        //   ff[f]=sum;
        if (add(all, f)) {
            all.push_back(f);
        }
    ...
    for (int i=0;i<all.size();i++){
        std::cout<<all[i].getStreet()<<" "<<all[i].getCity()<<" - "<<getTotalPrice(all[i])<<std::endl;
