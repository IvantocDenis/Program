...
protected:
    bool isChecked;
//    friend class LengthPolicy;
//    friend class ClassPolicy;
//    friend class IncludePolicy;
//    friend class NotIncludePolicy;
//    friend class RepetitionPolicy;
//    friend class ConsecutivePolicy;
public:
//    Policy(){
//        isChecked=0;
//    }
    virtual void check(const std::string & S)=0;
...
public:
    LengthPolicy(uint16_t m){
        minLength=m;
        maxLength=255;
    }
    LengthPolicy(uint16_t m,uint16_t ma){
        minLength=m;
        maxLength=ma;
    }
...
public:
    ...
    void check(const std::string &pass)override {
        int counter=0;
        bool l, u, d, s;
...
public:
    ...
    void check(const std::string & S)override{
        isChecked=1;
...
std::string checkPassword(std::string password, std::vector<Policy*> v){
...
int main() {
    ...
    for (int i = 0; i < n; i++) {
       ...
       // std::cout<<s<<std::endl;
        if (s == "length") {
         int kk= scanf ("%hu %hu",&a,&nn);
         if (kk==1){
             x.push_back(new LengthPolicy(a));
         }else{
             x.push_back(new LengthPolicy(a,nn));
         }
        }
        if (s == "class") {
            std::cin >> nn;
            x.push_back(new ClassPolicy(nn));
        }else
        if (s == "include") {
            std::cin >> ss;
            x.push_back(new IncludePolicy(ss));
        }else
        if (s == "ninclude") {
            std::cin >> ss;
            x.push_back(new NotIncludePolicy(ss));
        }else
        if (s == "repetition") {
            std::cin >> nn;
            x.push_back(new RepetitionPolicy(nn));
        }else
        if (s == "consecutive") {
            std::cin >> nn;
            x.push_back(new ConsecutivePolicy(nn));
        }
    ...
    while (std::cin >>s) {
        //std::cout<<s;
        std::cout <<checkPassword(s,x) << std::endl;
